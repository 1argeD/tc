 ...
 * */
            /*e의 값이 협약인 limit 값보다 같거나 작다면 e를 반환하고 아니라면 power 를 반환해 int 로 바꾼 후 철의 무게를 계산하기 위해 총합을 계산한다 */
            return list.stream()
                    .mapToInt(e -> e <= limit ? e : power)
        ...
        private void countDivisor(List<Integer> list, int number) {
            ...
            for (int i = 1; i <= Math.sqrt(number); i++) {/*sqrt()제곱근 계산 음수의 경우 오류를 리턴함*/
                if (number % i == 0) {
                    count++;
                    if (i != number / i) {
                        count++;
